...
app.get("/api/crosscheck", async (req, res) => {
  ...
  try {
    ...
    // 1. Check by microchip (highest priority for animals)
    if (microchip) {
    ...
    // 2. Check by exact profile (email, phone, or fiscal code)
    ...
    // 3. Check for name duplicates (people)
    if (nome && cognome) {
    ...
    // 4. Check for animal name & species duplicates to avoid duplicate entry
